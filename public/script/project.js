...
plusButton.addEventListener('click', (e) => {
  e.preventDefault();
  window.name = "parentPage";
  child_window = window.open("/upload/project", "글 쓰기", status_);
});
...
$(document).on("click", "#project-card-author-anchor", (e) => {
  e.preventDefault();

  let nickname = e.target.innerText.substring(4, e.target.innerText.length);
...
$(document).on("click", (e) => {
  if(userTab.style.display === 'block' && e.target.id !== 'project-card-author-anchor' && e.clientX > 400) {
